...
Reference guide for how the NestJS Code Review skill processes different input types.
...
**Requirements**:
- Must be run from within NestJS project repository
...
✅ https://gitlab.com/myorg/backend/-/merge_requests/89
...
#### Ambiguous Input
```
❌ Input: auth  (too vague, multiple matches)
Response: "Multiple branches match 'auth':
          • feature/auth-system
...
git diff --name-only main..feature/auth-redesign

# 5. Get change statistics
git diff --stat main..feature/auth-redesign
...
git diff --name-only main..feature/auth-system

# 5. Get change statistics
git diff --stat main..feature/auth-system
```
...
```
Source Branch: feature/auth-redesign (user's work)
Destination Branch: main (target branch from PR)

Diff: main → feature/auth-redesign
(Shows exactly what PR will merge)

Report Title: "PR #427: Auth Redesign Review"
  Source: feature/auth-redesign
  Target: main
...
```
Source Branch: feature/auth-system (user's work)
Destination Branch: main (assumed default)

Diff: main → feature/auth-system
(Shows what would be merged if PR created)

Report Title: "Branch Review: feature/auth-system"
